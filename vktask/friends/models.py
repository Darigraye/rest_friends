...
class UserModel(AbstractUser):
    ...
    friends: ManyToManyField = ManyToManyField(
        "UserModel", blank=True, symmetrical=True, verbose_name="друзья"
    )
...
class FriendRequestModel(Model):
    ...
    receiver: ForeignKey = ForeignKey(
        UserModel,
        on_delete=DO_NOTHING,
    ...
    content: CharField = CharField(
        max_length=50, blank=True, verbose_name="сопроводительный текст заявки"
    )
    ...
    class Meta:
        ...
        constraints = [
            CheckConstraint(
                check=~Q(receiver_id=F("sender_id")), name="check_the_same_user"
            ),
        ]
